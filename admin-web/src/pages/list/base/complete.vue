    ...
    <card class="list-card-container">
      ...
      >
        ...
        <template #invoiceType="{ row }">
          <p v-if="row.invoiceType === CONTRACT_TYPES.MAIN">{{ invoiceTypeCode[row.invoiceTypeCode] }}</p>
        </template>
        <template #op="slotProps">
          <t-button v-if="slotProps.row.state === 2" size="small" @click="edit(slotProps.row)">查看发票</t-button>
        </template>
      </t-table>
    </card>
  </div>
...
import { COLUMNS } from './constants1';
