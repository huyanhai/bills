<template>
  <view class="invoice-info">
    <image :src="infoData.redPdfImgUrl" class="pdf" v-if="infoData.redPdfImgUrl" @click="previewImage(infoData.redPdfImgUrl)" />
    <van-button type="info" @click="goPath(`Red?id=${id}&type=red`)" class="btn">查看明细</van-button>
    ...
    <van-dialog use-slot :show="show1" confirmButtonText="复制" show-cancel-button @confirm="confirmDown" :before-close="beforeClose" @cancel="show1 = false">
      <view style="margin: 10px 0">
        <view class="down">
          <text>下载地址</text>
          <view class="url">{{ infoData.redPdfUrl }}</view>
...
export default {
  ...
  methods: {
    ...
    confirmDown() {
      let url = `https://pmy.funku.cn/agent/api/p/download/redpdf/${this.id}`;
      const vm = this;
